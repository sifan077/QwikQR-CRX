<div align="center">
    <img src="https://github.com/sifan077/QwikQR-CRX/blob/master/logo/logo-128.png"/>
    <h1>📸 QwikQR - Chrome QR Code Extension</h1>
    <p>一个简单的 Chrome 扩展，让二维码生成和识别变得简单快捷</p>
</div>

## ✨ 功能特色

- **🔗 URL 二维码生成** - 将当前网页链接快速转换为二维码
- **📝 文本二维码生成** - 输入任意文本内容生成二维码
- **📤 链接/文本二维码生成** - 右键菜单快速生成选中内容的二维码
- **📥 二维码识别** - 从图片中提取和解析二维码内容
- **📷 截图识别** - 右键菜单截图识别任意区域的二维码
- **💾 下载功能** - 保存生成的二维码为 PNG 图片
- **📋 复制功能** - 复制二维码图片到剪贴板
- **📜 历史记录** - 自动保存生成的二维码，支持快速复用和管理
- **🌙 深色模式** - 支持深色主题，保护眼睛
- **🌐 支持无协议URL** - 识别如 `www.baidu.com` 等常见域名格式
- **📱 实时生成** - 输入内容时实时生成二维码
- **⚙️ 自定义设置** - 可自定义二维码大小、颜色、纠错等级等参数
- **🎨 中心 Logo** - 支持在二维码中心添加自定义 Logo 图片

## 🚀 安装指南

- **Fork** 这个仓库，然后 **克隆你分叉的仓库到本地**。如果你没有 GitHub 账号，可以直接下载仓库的 zip 文件并解压到你的电脑上。
- **打开 [扩展程序页面](chrome://extensions)** 在你的浏览器中：`chrome://extensions`。这个链接在任何基于 Chromium 的浏览器中都有效。
- 如果你还没有开启，**打开 "开发者模式"**。这通常是在扩展页面右上角的一个切换按钮。
- 点击 **_加载已解压的扩展程序_** 按钮。
- 在弹出的窗口中，**选择包含此扩展的文件夹**，然后 **点击 _确定_**。
- **完成！** 一个名为 _QwikQR_ 的新扩展应该出现在列表中。

## 🎯 使用方法

### 生成二维码
1. 点击浏览器工具栏中的 QwikQR 图标
2. 在弹出的界面中：
   - 默认显示当前网页的 URL，已生成二维码
   - 或者输入自定义内容到文本框，二维码会实时生成
   - 右键选中文本或链接，选择 "生成二维码" 选项

### 识别二维码
1. 右键点击包含二维码的图片
2. 选择 "识别二维码" 选项
3. 扩展会自动识别二维码内容并显示操作选项

### 截图识别二维码
1. 在任意页面右键
2. 选择 "截图识别二维码" 选项
3. 按住鼠标左键拖动选择包含二维码的区域
4. 松开鼠标后自动识别并显示结果
5. 可以复制内容或打开链接（如果是 URL）
6. 按 ESC 键可取消选择

### 自定义设置
1. 点击弹出窗口右下角的设置按钮 ⚙️
2. 在设置页面中可以自定义：
   - 二维码大小（100-300 像素）
   - 前景色和背景色
   - 纠错等级（L/M/Q/H）
   - 生成后的默认操作（复制/下载）
   - 中心 Logo 图片
3. 点击"保存设置"按钮应用更改

### 添加中心 Logo
1. 打开设置页面
2. 在"二维码生成"部分找到"中心 Logo 图片"
3. 点击"选择图片"上传你的 Logo（建议使用正方形 PNG 图片）
4. 调节 Logo 大小（10%-30%）
5. 保存设置
6. 之后生成的二维码会自动在中心显示 Logo

### 使用历史记录
1. 点击弹出窗口底部的历史记录按钮 📋
2. 查看已生成的二维码历史记录（最多保存 50 条）
3. 点击任意历史记录项可快速重新生成该二维码
4. 点击记录右侧的 × 按钮可删除单条记录
5. 点击底部的"清除全部"按钮可删除所有历史记录
6. 在设置页面可以查看历史记录数量并清除所有记录

### 深色模式
1. 打开设置页面
2. 在"外观"部分找到"深色模式"开关
3. 打开开关即可切换到深色主题
4. 点击"保存设置"按钮保存更改
5. 深色模式会自动应用到弹出窗口和设置页面

## 📋 支持的二维码类型

- 网页链接（支持带/不带协议的 URL）
- 纯文本内容
- 邮箱地址

## 🚧 TODO 功能（计划中）

- **🔍 批量识别** - 一次性识别多张图片中的二维码
- **📤 分享功能** - 直接分享生成的二维码
- **🔍 更精确的识别** - 改进二维码识别算法的准确性

## 🛠 技术特性

- **Manifest V3** - 使用最新的 Chrome 扩展规范
- **现代化 UI** - 美观且用户友好的界面设计
- **高效算法** - 快速的二维码生成和识别
- **安全可靠** - 本地处理，不上传任何数据

## 🤝 贡献

欢迎提交 Issue 和 Pull Request 来帮助改进这个项目！

## 📄 许可证

此项目基于 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件了解详情。

## 🙏 鸣谢

感谢 [jsQR](https://github.com/cozmo/jsQR) 库提供的二维码识别能力，以及 [qrcode.js](https://github.com/davidshimjs/qrcodejs) 库的二维码生成功能。

</div>